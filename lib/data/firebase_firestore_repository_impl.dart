...
class FirebaseFirestoreRepositoryImpl implements FirebaseFirestoreRepository {
  ...
  static const String seasonDataCollection = 'seasonData';
  ...
  CollectionReference<Map<String, dynamic>> get seasonDataCol =>
      FirebaseFirestore.instance.collection(seasonDataCollection);
  ...
  Future<void> addZ1UserAward(Z1UserAward z1UserAward) async {
    final awards = currentUserNotifier.value?.awards ?? {};
    awards[z1UserAward.trackId] = z1UserAward;
    currentUserNotifier.value = currentUserNotifier.value?.copyWith(
      awards: awards,
    );
    return userDoc.update({
      'awards': currentUserNotifier.value?.toJson()['awards'] ?? {},
    });
  }
  ...
  Future<List<Z1Track>> getTracksByIds({required List<String> trackIds}) async {
    ...
    final query = trackDataCol.where('trackId', whereIn: trackIds);

    final qSnap = await query.get();
    if (qSnap.docs.isNotEmpty) {
      final tracks =
          qSnap.docs.map((e) => Z1Track.fromMap(e.data())).toList().sorted(
  ...
  Future<List<Z1Season>> getSeasons() async {
    ...
    final query = seasonDataCol;

    final qSnap = await query.get();
    if (qSnap.docs.isNotEmpty) {
      final seasons = qSnap.docs
          .map((e) => Z1Season.fromMap(e.data()))
          .toList()
